function gen_prcp_mat(file_name)
  % --------------- Intro ---------------- % 
  % (importing, parsing, constants and shit)

  % Read in data
  raw_data = importdata(strcat('../data/', file_name, '_Weather.txt'));
  ...
  % Compute equilibrium distribution matrix
  equilibrium_distribution_matrix = prcp_matrix^40;
  ...
  % Write result files
  xlswrite(strcat('../results/prcp_', file_name, '.xls'), prcp_matrix);
  xlswrite(strcat('../results/prcp_', file_name, '_equilibrium.xls'), equilibrium_distribution_matrix);
end
